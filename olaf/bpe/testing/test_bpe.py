...
# for embbedded decoding (?)
        # desired compression ratio (bits per pixel) - uncommon for decoding; default is 0
        self._bpp = 14  
    ...
    # less common specs (for encoding only)
        ...
        self._enc_data = bpe.encode_basearg(self._infile, self._compr_file, 
                                            self._width, self._height, 
                                            self._bpp, self._bit_depth)
        self._enc_data = bpe.encode_fullarg(self._infile, self._compr_file, 
                            self._width, self._height, self._bpp, 
                            self._bit_depth, self._blocks_per_seg, 
                            self._is_big_endian, self._is_lossless)
